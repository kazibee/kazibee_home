...
(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const c of document.querySelectorAll('link[rel="modulepreload"]'))i(c);new MutationObserver(c=>{for(const s of c)if(s.type==="childList")for(const f of s.addedNodes)f.tagName==="LINK"&&f.rel==="modulepreload"&&i(f)}).observe(document,{childList:!0,subtree:!0});function n(c){const s={};return c.integrity&&(s.integrity=c.integrity),c.referrerPolicy&&(s.referrerPolicy=c.referrerPolicy),c.crossOrigin==="use-credentials"?s.credentials="include":c.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(c){if(c.ep)return;c.ep=!0;const s=n(c);fetch(c.href,s)}})();const Vt="modulepreload",$t=function(e,t){return new URL(e,t).href},ft={},qt=function(t,n,i){let c=Promise.resolve();if(n&&n.length>0){const f=document.getElementsByTagName("link"),w=document.querySelector("meta[property=csp-nonce]"),b=(w==null?void 0:w.nonce)||(w==null?void 0:w.getAttribute("nonce"));c=Promise.allSettled(n.map(g=>{if(g=$t(g,i),g in ft)return;ft[g]=!0;const y=g.endsWith(".css"),I=y?'[rel="stylesheet"]':"";if(!!i)for(let x=f.length-1;x>=0;x--){const S=f[x];if(S.href===g&&(!y||S.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${g}"]${I}`))return;const R=document.createElement("link");if(R.rel=y?"stylesheet":Vt,y||(R.as="script"),R.crossOrigin="",R.href=g,b&&R.setAttribute("nonce",b),document.head.appendChild(R),y)return new Promise((x,S)=>{R.addEventListener("load",x),R.addEventListener("error",()=>S(new Error(`Unable to preload CSS for ${g}`)))})}))}function s(f){const w=new Event("vite:preloadError",{cancelable:!0});if(w.payload=f,window.dispatchEvent(w),!w.defaultPrevented)throw f}return c.then(f=>{for(const w of f||[])w.status==="rejected"&&s(w.reason);return t().catch(s)})},vt=globalThis;typeof vt.global>"u"&&(vt.global=globalThis);/*! *****************************************************************************
...
${e}`:Ee,Object.assign(t.style,{whiteSpace:"pre-wrap",alignItems:"center",background:"#111827",color:"#f8fafc",display:"flex",font:"500 16px system-ui, sans-serif",inset:"0",justifyContent:"center",padding:"24px",position:"fixed",textAlign:"center",zIndex:"2147483647"}),document.body.appendChild(t)}function sr(e){return new Map(e.map(t=>[t.channel,{channel:t.channel,controller:t.controller,action:t.action,...t.hasInput?{inputSchema:{type:"object",properties:{payload:{}}}}:{}}]))}async function cr(){ar();const e=new WebSocket(`wss://${location.host}/v1/agent/session`),t=new Map,n=new Map,i=St(),c=St();let s=!1;const f=(y=Ee)=>{if(!s){s=!0;for(const I of t.values())I.reject(new Error(y));t.clear(),i.reject(new Error(y)),c.reject(new Error(y)),Ut(y)}};e.addEventListener("message",y=>{var R;let I;try{I=JSON.parse(String(y.data))}catch{f("Remote renderer returned a malformed frame"),e.close(4400,"malformed frame");return}if("kind"in I){I.kind==="session.ready"&&i.resolve(),I.kind==="session.closed"&&f(I.reason);return}if(I.type==="ready"){c.resolve();return}if(I.type==="event"){(R=n.get(I.id))==null||R(I.payload);return}const O=t.get(I.id);O&&(t.delete(I.id),I.type==="error"?O.reject(new Error(I.message)):O.resolve(I.value))}),e.addEventListener("close",()=>f()),e.addEventListener("error",()=>f());const w=y=>{if(s||e.readyState!==WebSocket.OPEN)throw new Error("Remote renderer session is not connected");e.send(JSON.stringify(y))},b={invoke(y,I){const O=at("invoke");return new Promise((R,x)=>{t.set(O,{resolve:R,reject:x});try{w({type:"invoke",id:O,channel:y,...I===void 0?{}:{payload:I}})}catch(S){t.delete(O),x(S)}})},subscribe(y,I){const O=at("subscription");return n.set(O,I),w({type:"subscribe",id:O,channel:y}),()=>{n.delete(O),!s&&w({type:"unsubscribe",id:at("unsubscribe"),subscriptionId:O})}}};await i.promise,await c.promise;const{bridge:g}=nr(sr(ir(rr)),b);Object.assign(g,{__window:{current:async()=>({ok:!0,windowId:or,defaultRoute:"main.start"}),open:async()=>{},close:async()=>{},focus:async()=>{},minimize:async()=>{},maximize:async()=>{}},__woodConfig:async()=>({appEnv:"production",rendererTraceEnabled:!1,rendererTraceBatchEnabled:!1}),__navigate:async(y,I)=>{Hn().go(I)},__contextMenu:{show:async()=>{}},__load:async()=>{throw new Error("Route loaders are unavailable in the web agent")},__debug:{current:async()=>({runtime:"web-agent"})},__log:(y,I,O,R)=>{(y==="error"?console.error:y==="warn"?console.warn:y==="debug"?console.debug:console.info)(`[${I}] ${O}`,R)},__trace:()=>{}}),globalThis.__NOEGO_WOOD_BRIDGE__=g,globalThis.wood=g,await qt(()=>import("./main.js").then(y=>y.m),__vite__mapDeps([0,1]),import.meta.url)}cr().catch(e=>{console.error("[web-agent] bootstrap failed",e);const t=e instanceof Error?`${e.name}: ${e.message}`:String(e);Ut(`Bootstrap failed — ${t}`)});export{Ht as C,Jt as E,kt as I,Z as L,$n as N,V as O,lr as P,Pe as S,Oe as T,qt as _,Me as a,Se as b,Te as c,it as d,ht as e,Re as f,Hn as g,me as h,Et as i,Ot as j,dr as k,ur as s};
