CREATE TABLE connect_desktop_devices (
  device_id TEXT PRIMARY KEY CHECK (device_id ~ '^dev_[A-Za-z0-9]{8,64}$'),
  ...
  desktop_version TEXT NOT NULL, key_fingerprint TEXT NOT NULL CHECK (key_fingerprint ~ '^[a-f0-9]{64}$'),
...
CREATE INDEX idx_connect_desktop_devices_owner_state ON connect_desktop_devices(owner_user_id, state, created_at);
