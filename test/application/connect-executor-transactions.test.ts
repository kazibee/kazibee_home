/**
 * ConnectExecutorLogic @transaction methods against REAL SQL through the
 ...
 * revoke paths, including the pending re-claim refresh branch and the
 * decide() serialization queue. Historical case names and assertions retained.
 */
...
import {
  insertAccount,
  insertAcceptedExecutor,
  insertPendingClaim,
} from "../schemas/executor-transaction-data";
...
describe("ConnectExecutorLogic transactional paths against real SQL", () => {
  it("createClaim persists the executor + claim rows and returns the challenge envelope", resourceCase(async () => {
  ...
  it("createClaim retries the same envelope, conflicts on drift, and refreshes a pending re-claim", resourceCase(async () => {
    const built = await testPostgres(executorRegistrySchema, {
      adminUrl: process.env.SQLSTACK_TEST_PG_URL ?? DEFAULT_ADMIN_URL,
    }).build();
    const env = await testApp(CONFIG).select({ server: { module: ["connectExecutors"] } })
    ...
    // Starting state: executor 001 registered with its pending claim (plain rows).
    const input = createInput("001");
  ...
  it("decide(accept) transitions claim + executor rows, mints a generation-1 credential, and audits", resourceCase(async () => {
    const built = await testPostgres(executorRegistrySchema, {
      adminUrl: process.env.SQLSTACK_TEST_PG_URL ?? DEFAULT_ADMIN_URL,
    }).build();
    const env = await testApp(CONFIG).select({ server: { module: ["connectExecutors"] } })
  ...
  it("decide(accept) replay is idempotent for the same key and replayed for a different key", resourceCase(async () => {
    ...
    await insertAcceptedExecutor(
      built, createInput("002"), TOKEN_B, USER_ID, decisionInput("002", "accept").idempotencyKey,
    );
  ...
  it("rename updates the owned active executor row and audits executor.renamed", resourceCase(async () => {
    const built = await testPostgres(executorRegistrySchema, {
      adminUrl: process.env.SQLSTACK_TEST_PG_URL ?? DEFAULT_ADMIN_URL,
    }).build();
    const env = await testApp(CONFIG).select({ server: { module: ["connectExecutors"] } })
    ...
    // Starting state: executor 002 active and owned by USER_ID (plain rows).
    await insertAcceptedExecutor(
      built, createInput("002"), TOKEN_B, USER_ID, decisionInput("002", "accept").idempotencyKey,
    );
  ...
  it("revoke fences the credential generation, revokes credentials, audits, and is idempotent", resourceCase(async () => {
    ...
    // Starting state: executor 002 active, owned by USER_ID, one active
  ...
  it("atomically selects exactly one owner when two authenticated accounts race", resourceCase(async () => {
    ...
    await insertAccount(built, "usr_dbexecrace2");
